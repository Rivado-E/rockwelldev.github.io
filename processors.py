...
from models import ContentItem, TwitterEngagements
...
class TwitterFeedProcessor:
    ...
    def _extract_urls(
        ...
        if "urls" in entities:
            for url_dict in entities["urls"]:
                if not url_dict["expanded_url"].startswith("https://twitter.com/"):
                    ...
                    # NOTE: remove the url from the text?
                    # if it exists
        ...
        # NOTE: Now what do we do with the urls once we remove them from the text
        # and save them to the urls list, I also need the format of the new url list
        return (
            text.strip(),
